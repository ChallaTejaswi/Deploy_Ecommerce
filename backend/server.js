...
app.post('/api/webhook', optionalAuth, async (req, res) => {
  try {
    ...
    } else if (normalizedIntent === 'recommendation' || normalizedIntent === 'suggest' || normalizedIntent === 'recommendationintent'||normalizedIntent === 'ProductRecommendationIntent') {
    try {
        const { handleRecommendations } = require('./services/enhancedWebhook');
        const recommendResult = await handleRecommendations(userId, { languageCode });
        productResults = recommendResult.products;
        responseText = recommendResult.responseText;
    } catch (error) {
        console.error('Recommendation error:', error);
        responseText = `❌ Sorry, I'm having trouble getting recommendations right now.`;
        productResults = [];
    }
}
    else {
