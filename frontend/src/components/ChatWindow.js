    ...
    try {
      ...
      if (isRecommendationRequest && isAuthenticated && user) {
        try {
          ...
          // Extract recommendations from the API response - backend returns { recommendations: [...] }
          const recommendedProducts = recommendationsResponse.data?.recommendations || [];
          ...
          if (recommendedProducts.length > 0) {
            ...
            // Optional: Show a message when no recommendations are available
            const noRecommendationsMessage = {
              id: Date.now() + 1,
              text: `🤔 I don't have enough data to make personalized recommendations yet. Try browsing some products first, and I'll learn your preferences!`,
              sender: 'bot',
              timestamp: new Date()
            };
